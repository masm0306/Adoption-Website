...
if($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['email']) && !isset($_GET['password'])) {
	...
	if($db) {
			...
			$query  = "SELECT id,name,email FROM petlovers where email='" . mysqli_real_escape_string($db, trim($_GET['email'])) . "'";
			// executar a query
...
if($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['email']) && isset($_GET['password'])) {
	...
	if($db) {
		...
		$query  = "SELECT id, name, email FROM petlovers WHERE email='$email' AND password_digest='$password'";
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($db) {
		// criar query numa string
		$query  = "INSERT INTO petlovers SET name='$name',email='$email',password_digest='$password',created_at=NOW(),updated_at=NOW()";
		...
		// criar query numa string
		$query  = "SELECT id, name, email FROM petlovers order by id desc limit 1";
